...
void CAdvComboBox::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    else if( (GetStyle() & CBS_DROPDOWN) && (GetStyle() & CBS_SIMPLE) )	// == CBS_DROPDOWNLIST
    {
        CRect rc = m_rcCombo;
        GetClientRect( &rc );
...
void CAdvComboBox::ModifyACBStyle(UINT nRemoveStyle, UINT nAddStyle)
{
    ...
    else if( nAddStyle & ACBS_STANDARD )
    {
...
int CAdvComboBox::SetCurSel(int nSelect)
{
    ...
    else if( nSelect < -1 || nSelect > (int)m_list.size()-1 )
    {
        return CB_ERR;
    }
    else
