...
#include "StdAfx.h"
#include "..\header\ATA_DLL_TestItem_TCard.h"

CTestItem_TCard::CTestItem_TCard(ATA_DLL_Handle *current_handle, int timeout, E_ATDLL_TEST_ITEM_COUNT item, char *testName)
    : CTestItem_Interface(current_handle, timeout, item, testName)
{
}

CTestItem_TCard::~CTestItem_TCard()
{
}


E_ATADLL_RESULT CTestItem_TCard::StartTest ()
{
	E_ATADLL_RESULT testResultCode;
    ClearATResPool();
    test_handle->m_commonCFG.cbTestProgess (test_handle->handle_index, E_TCARD, 10);
    ...
    if (IsAutoMode())
    {
        atstr = "AT+MEMCARD=1,1";
    }
    else
    {
        atstr = "AT+MEMCARD=1,2";
    }
#else
    atstr = "AT+MEMCARD";
#endif
